...
export default class App extends Component {
  ...
  handleDisplayChange() {
    ...
    if (endYear.length != 4 || startYear.length != 4){
      alert("Invalid year selection.\nPlease make sure you use a valid 4 digit year.");
    }
        ...
        return (
            <div id="pagewrapper" className="ui container">
              <div className="ui top attached menu">
                <a className="item" onClick={this.handlePageChange.bind(this, 2)}><i
                    className="calendar plus icon"></i> New Event</a>
                <a className="item" onClick={this.handlePageChange.bind(this, 3)}><i className="user icon"></i> User
                  Profile</a>
              ...
              <div className="ui grid">
                <div id="date-select" className="sixteen wide column">
                  <p>Display Events</p>
                  From
                  <select defaultValue={new Date(Date.now()).getMonth()} ref="displayStartMonth"
                  ...
                  <button className="ui basic button" onClick={this.handleDisplayChange.bind(this)}><i
                      className="calendar alternate icon"></i>Display
                  </button>
                </div>
                {this.renderDays()}
              </div>
      ...
      case 2: //add-modify page
        return (
            <div className="ui container">
              <div className="ui top attached menu">
                <a className="item" onClick={this.handlePageChange.bind(this, 1)}><i
                    className="calendar alternate icon"></i> Event Calendar</a>
                <a className="item" onClick={this.handlePageChange.bind(this, 3)}><i className="user icon"></i> User
                  Profile</a>
                <div className="ui right menu">
                  <a className="item" onClick={this.handleLogout.bind(this)}><i className="logout icon"></i> Logout</a>
                ...
                <div className="row">
                  <div className="ui grid">
                    ...
                    <div className="ten wide column">
                      ...
                      <div className="ui input"><input ref="eventDateYear" className="ui input"
                                                       defaultValue={this.getModYear()} placeholder='Year'
                                                       style={{ width: 70 + 'px' }}/>
                      </div>
                    </div>
                  </div>
            ...
            <div id="pagewrapper" className="ui container">
              ...
              <div className="ui centered grid">
                <div className="five wide column">
                  ...
                  <div className="ui input"><input type="password" ref="oldPassword" placeholder="Your old password"/>
                  </div>


                  <div className="ui input"><input type="password" ref="changePassword"
                                                   placeholder="Your new password"/></div>


                  <div className="ui input"><input type="password" ref="changePasswordConfirm"
    ...
    if (id !== allEvents[event]._id) {   //if we are modifying an event, don't compare to self
      if (allEvents[event].room === room) {  //check if we are in the same room before comparing
        if ((allEvents[event].startTime.getTime() <= startTime.getTime()) &&
            (allEvents[event].endTime.getTime() >= startTime.getTime())) { // E1s < E2s < E1e
          return true;
        } else
          if ((allEvents[event].startTime.getTime() <= endTime.getTime()) &&
            ...
            if ((allEvents[event].startTime.getTime() >= startTime.getTime()) &&
                (allEvents[event].endTime.getTime() <= endTime.getTime())) { //E2s > E1s & E1e > E2e
              return true;
            }
      }
    }
...
//Event is before yesterday's date(year, month, day): -2
...
//This function deletes all events that are before the yesterday's date/time.
